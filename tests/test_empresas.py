...
def test_criar_empresa(client, novo_cnpj):
    payload = {
...
def test_cnpj_duplicado(client, novo_cnpj):
    payload = {"nome_empresa": "Duplicada", "cnpj": novo_cnpj}
    primeira = client.post("/api/v1/empresas", json=payload)
    assert primeira.status_code == 201
    id_empresa = primeira.json()["id_cliente"]

    segunda = client.post("/api/v1/empresas", json=payload)
    assert segunda.status_code == 400
    assert "já está cadastrado" in segunda.json()["detail"]

    # limpeza
    client.delete(f"/api/v1/empresas/{id_empresa}")
...
def test_atualizar_empresa(client, empresa_teste):
    id_cliente = empresa_teste["id_cliente"]
    novo_nome = "Nome Atualizado"
    payload = {
        "nome_empresa": novo_nome,
        "cnpj": empresa_teste["cnpj"],  # mesmo CNPJ
        "ids_servicos_contratados": []
    }
    response = client.put(f"/api/v1/empresas/{id_cliente}", json=payload)
    assert response.status_code == 200
    data = response.json()
    assert data["nome_empresa"] == novo_nome

def test_excluir_empresa(client, novo_cnpj):
    ...
    delete_resp = client.delete(f"/api/v1/empresas/{id_empresa}")
