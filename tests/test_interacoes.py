def test_criar_interacao(client, empresa_teste):
    payload = {
    ...
    resp = client.post("/api/v1/interacoes", json=payload)
    assert resp.status_code == 200
    data = resp.json()
    assert data["tipo_interacao"] == "visita"
    # limpeza
    client.delete(f"/api/v1/interacoes/{data['id_interacao']}")
...
def test_atualizar_interacao(client, empresa_teste):
    ...
    put_resp = client.put(f"/api/v1/interacoes/{id_interacao}", json=update)
    assert put_resp.status_code == 200
    assert put_resp.json()["feedback_anotacoes"] == "Atualizado"

    client.delete(f"/api/v1/interacoes/{id_interacao}")

def test_interacoes_pagas(client, empresa_teste):
    # Cria interação paga
    payload = {
        "id_cliente": empresa_teste["id_cliente"],
        "tipo_interacao": "visita",
    ...
    client.delete(f"/api/v1/interacoes/{id_int}")