...
def criar_pagamento(pagamento_in: PagamentoCreate, db: Session = Depends(get_db)):
    try:
...
def atualizar_pagamento(id_pagamento: UUID, pagamento_in: PagamentoCreate, db: Session = Depends(get_db)):
    try:
